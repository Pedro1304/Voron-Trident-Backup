...
|------|-------------|------|-------|
| 2026-07-18 | ~270.8h* | Recurrence of zero-fill doc corruption — root cause found & fully closed | First post-decoupling autobackup sync (`e417553`) pushed `Maintenance_Log.md`/`Trident_Build_Info.md` to GitHub as 100% null bytes, byte-identical in size (12638B/16620B) to the original 2026-06-27 incident. Root cause: that earlier incident was **only ever recovered on GitHub** — the live copies on `~/printer_data/config` were never actually confirmed/fixed on disk, and sat zero-filled for 3 weeks undetected because autobackup was down the whole time (printer off). Tonight's first successful sync simply mirrored that dormant corruption up to GitHub for the first time. Not a new SD-card failure — `dmesg` showed the same `error -5` UHS-tuning retry as before but it resolved normally on retry, filesystem mounted clean. **Fix:** recovered GitHub copies from last-good blob (`810b0d2`, commit `e42b28d`), then copied the good content back onto the live disk (`~/printer_data/config/`) via the decoupled repo, restarted `voron-backup.service`. Verified stable across two further sync cycles (`03fb15e`, `7d15520`) — both clean, no recurrence. **Lesson for next time:** after any live-file recovery, always verify the fix landed on the live disk, not just in GitHub — don't rely on the next autobackup cycle to confirm. |
